...
# remove time series with less than 1 Myr interval
df <- subset(df, interval_MY >= 1)
...
load(file = paste0(PATH, "model_test_<1Myr.Rdata"))
load(file = paste0(PATH, "model_test_meta_<1Myr.Rdata"))
...
darwins_compl_lmer_plot <- ggplot(bind_darwins_compl, aes(interval_MY, darwins)) + 
  ...
  annotate("text", x = -5, y = -3.5, parse = TRUE, label="italic(y)==-2.096-1.044~italic(x)", size = 5) +
  annotate("text", x = -5, y = -5, parse = TRUE, label="italic(SE)=='' %+-% '0.142'", size = 5) +
  annotate("text", x = -5, y = -6.5, parse = TRUE, label = "italic(R)^2== 0.398", size = 5) +
  annotate("text", x = -5, y = -8, parse = TRUE, label = "italic(n)==424", size = 5) +
...
darwins_rel_lmer_plot <- ggplot(bind_darwins_rel, aes(interval_MY, darwins)) + 
  ...
  ylab(expression(paste("Log ", italic("darwins")))) + xlab(("Log time")) +
  annotate("text", x = -5, y = -3, parse = TRUE, label="italic(y)==-2.012-0.896~italic(x)", size = 5) +
  annotate("text", x = -5, y = -4.5, parse = TRUE, label="italic(SE)=='' %+-% '0.152'", size = 5) +
  annotate("text", x = -5, y = -6, parse = TRUE, label = "italic(R)^2== 0.350", size = 5) +
  annotate("text", x = -5, y = -7.7, parse = TRUE, label = "italic(n)== 127", size = 5) +
...
darwins_abs_lmer_plot <- ggplot(bind_darwins_abs, aes(interval_MY, darwins)) + 
  ...
  ylab(expression(paste("Log ", italic("darwins")))) + xlab(("Log time")) +
  annotate("text", x = -5, y = -3, parse = TRUE, label="italic(y)==-2.136-0.850~italic(x)", size = 5) +
  annotate("text", x = -5, y = -4.5, parse = TRUE, label="italic(SE)=='' %+-% '0.159'", size = 5) +
  annotate("text", x = -5, y = -6, parse = TRUE, label = "italic(R)^2== 0.334", size = 5) +
  annotate("text", x = -5, y = -7.7, parse = TRUE, label = "italic(n)== 114", size = 5) +
...
URW_compl_no_error_lmer_plot <- ggplot(bind_URW_compl_no_error, aes(interval_MY, vstep)) +
  ...
  annotate("text", x = -5, y = -6, parse = TRUE, label="italic(y)==-1.999-0.915~italic(x)", size = 5) +
  annotate("text", x = -5, y = -8, parse = TRUE, label="italic(SE)=='' %+-% '0.194'", size = 5) +
  annotate("text", x = -5, y = -10, parse = TRUE, label = "italic(R)^2== 0.188", size = 5) +
  annotate("text", x = -5, y = -12.2, parse = TRUE, label = "italic(n)== 430", size = 5) +
...
URW_rel_no_error_lmer_plot <- ggplot(bind_URW_rel_no_error, aes(interval_MY, vstep)) + 
  ...
  annotate("text", x = -5, y = -4.5, parse = TRUE, label="italic(y)==-2.406-0.985~italic(x)", size = 5) +
  annotate("text", x = -5, y = -6, parse = TRUE, label="italic(SE)=='' %+-% '0.216'", size = 5) +
  annotate("text", x = -5, y = -7.5, parse = TRUE, label = "italic(R)^2== 0.257", size = 5) +
  annotate("text", x = -5, y = -9.2, parse = TRUE, label = "italic(n)== 128", size = 5) +
...
URW_abs_no_error_lmer_plot <- ggplot(bind_URW_abs_no_error, aes(interval_MY, vstep)) + 
  ...
  ylab(expression(paste("Log ", italic(v)["step"]))) + xlab(expression("Log time")) +
  annotate("text", x = -5, y = -4.5, parse = TRUE, label="italic(y)==-2.407-0.997~italic(x)", size = 5) +
  annotate("text", x = -5, y = -6, parse = TRUE, label="italic(SE)=='' %+-% '0.227'", size = 5) +
  annotate("text", x = -5, y = -7.5, parse = TRUE, label = "italic(R)^2== 0.270", size = 5) +
  annotate("text", x = -5, y = -9.2, parse = TRUE, label = "italic(n)== 114", size = 5) +
...
URW_compl_lmer_plot <- ggplot(bind_URW_compl, aes(interval_MY, vstep)) + 
  ...
  annotate("text", x = -5, y = -6.2, parse = TRUE, label="italic(y)==-3.2512-0.989~italic(x)", size = 5) +
  annotate("text", x = -5, y = -8.2, parse = TRUE, label="italic(SE)=='' %+-% '0.187'", size = 5) +
  annotate("text", x = -5, y = -10.2, parse = TRUE, label = "italic(R)^2== 0.025", size = 5) +
  annotate("text", x = -5, y = -12.4, parse = TRUE, label = "italic(n)== 370", size = 5) +
...
URW_rel_lmer_plot <- ggplot(bind_URW_rel, aes(interval_MY, vstep)) + 
  ...
  ylab(expression(paste("Log ", italic(v)["step"]))) + xlab(expression("Log time")) +
  annotate("text", x = -5, y = -5, parse = TRUE, label="italic(y)==-3.199-1.030~italic(x)", size = 5) +
  annotate("text", x = -5, y = -6.5, parse = TRUE, label="italic(SE)=='' %+-% '0.200'", size = 5) +
  annotate("text", x = -5, y = -8, parse = TRUE, label = "italic(R)^2== 0.320", size = 5) +
  annotate("text", x = -5, y = -9.7, parse = TRUE, label = "italic(n)== 128", size = 5) +
...
URW_abs_lmer_plot <- ggplot(bind_URW_abs, aes(interval_MY, vstep)) + 
  ...
  ylab(expression(paste("Log ", italic(v)["step"]))) + xlab(expression("Log time")) +
  annotate("text", x = -5, y = -5, parse = TRUE, label="italic(y)==-3.223-1.032~italic(x)", size = 5) +
  annotate("text", x = -5, y = -6.5, parse = TRUE, label="italic(SE)=='' %+-% '0.208'", size = 5) +
  annotate("text", x = -5, y = -8.2, parse = TRUE, label = "italic(R)^2== 0.338", size = 5) +
  annotate("text", x = -5, y = -10, parse = TRUE, label = "italic(n)== 114", size = 5) +
...
pdf(width = 20.5, height = 11.5, file = "[PATH_TO_RESULTS_FOLDER]/empirical_>1Myr.pdf")
